...
def sample_random_memory(source_train_df: pd.DataFrame) -> pd.DataFrame:
    benign_df = source_train_df[source_train_df[LABEL_COL] == 0]
    attack_df = source_train_df[source_train_df[LABEL_COL] == 1]

    mem_benign = benign_df.sample(n=MEMORY_BENIGN, random_state=RANDOM_STATE, replace=False)
    mem_attack = attack_df.sample(n=MEMORY_ATTACK, random_state=RANDOM_STATE, replace=False)
...
def main() -> None:
    ...
    # =========================
    # Build random replay memory
    # =========================
    memory_df = sample_random_memory(source_train)

    print("\nReplay memory distribution:")
    print(memory_df[LABEL_COL].value_counts().sort_index())
    print("\nReplay memory attack breakdown:")
    print(memory_df[ATTACK_COL].value_counts().head(20))

    memory_df.to_csv(logs_dir / "random_replay_memory.csv", index=False)
    ...
    best_epoch = -1
    patience_counter = 0
    best_model_path = models_dir / "target_adapt_random_replay_best.pt"
    history = []
    ...
    print("\nAfter adaptation with random replay:")
    ...
    history_df.to_csv(logs_dir / "target_adaptation_random_replay_history.csv", index=False)

    metrics_df = pd.DataFrame(
        [
            {"phase": "pre_adapt_source_test", **pre_source_metrics},
            {"phase": "pre_adapt_target_test_zero_shot", **pre_target_metrics},
            {"phase": "post_adapt_target_test_random_replay", **post_target_metrics},
            {"phase": "post_adapt_source_test_random_replay", **post_source_metrics},
        ]
    )
    metrics_df.to_csv(logs_dir / "target_adaptation_random_replay_metrics.csv", index=False)

    print("\nSaved:")
    print(f"  adapted model: {best_model_path}")
    print(f"  history: {logs_dir / 'target_adaptation_random_replay_history.csv'}")
    print(f"  metrics: {logs_dir / 'target_adaptation_random_replay_metrics.csv'}")
