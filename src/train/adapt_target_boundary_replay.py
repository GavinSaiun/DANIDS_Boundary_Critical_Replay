...
MEMORY_SIZE = 5_000
MEMORY_BENIGN = MEMORY_SIZE // 2
MEMORY_ATTACK = MEMORY_SIZE // 2
...
def build_boundary_memory(
    source_train_df: pd.DataFrame,
    model: nn.Module,
    feature_cols: list[str],
    device: torch.device,
) -> pd.DataFrame:
    scored_df = source_train_df.copy()
    scored_df["boundary_score"] = score_boundary_closeness(model, scored_df, feature_cols, device)

    benign_df = scored_df[scored_df[LABEL_COL] == 0].copy()
    attack_df = scored_df[scored_df[LABEL_COL] == 1].copy()

    mem_benign = benign_df.nsmallest(MEMORY_BENIGN, "boundary_score")
    mem_attack = attack_df.nsmallest(MEMORY_ATTACK, "boundary_score")

    memory_df = pd.concat([mem_benign, mem_attack], axis=0).sample(
...
def main() -> None:
    ...
    print("\nBuilding boundary replay memory...")
    memory_df = build_boundary_memory(source_train, model, feature_cols, device)

    print("\nBoundary replay memory distribution:")
    print(memory_df[LABEL_COL].value_counts().sort_index())

    print("\nBoundary replay attack breakdown:")
    print(memory_df[ATTACK_COL].value_counts().head(20))

    print("\nBoundary score summary:")
    print(memory_df["boundary_score"].describe())

    memory_df.to_csv(logs_dir / "boundary_replay_memory.csv", index=False)
    ...
    best_epoch = -1
    patience_counter = 0
    best_model_path = models_dir / "target_adapt_boundary_replay_best.pt"
    history = []
    ...
    print("\nAfter adaptation with boundary replay:")
    ...
    history_df.to_csv(logs_dir / "target_adaptation_boundary_replay_history.csv", index=False)

    metrics_df = pd.DataFrame(
        [
            {"phase": "pre_adapt_source_test", **pre_source_metrics},
            {"phase": "pre_adapt_target_test_zero_shot", **pre_target_metrics},
            {"phase": "post_adapt_target_test_boundary_replay", **post_target_metrics},
            {"phase": "post_adapt_source_test_boundary_replay", **post_source_metrics},
        ]
    )
    metrics_df.to_csv(logs_dir / "target_adaptation_boundary_replay_metrics.csv", index=False)

    print("\nSaved:")
    print(f"  adapted model: {best_model_path}")
    print(f"  history: {logs_dir / 'target_adaptation_boundary_replay_history.csv'}")
    print(f"  metrics: {logs_dir / 'target_adaptation_boundary_replay_metrics.csv'}")
